...
db_file = st.text_input("Enter the SQLite database file path:", db_file)

widgetJsonPath = path + "/" + config["index_name"] + "_sql_saved_widgets.json"
...
def get_chat_response(schemas, request):
    ...
    From the request, generate 3 useful SQL queries. Return only JSON format with query_name and query_body. Each of them should be on a new line. SQL query should be formatted well
...
def getTablesAndSchemas(conn):
    ...
    # print(tables["name"], tables["name"])
    dic=dict()
    for selected_table in tables["name"]:
        
        schema_query = f"PRAGMA table_info({selected_table});"
        schema_df = pd.read_sql_query(schema_query, conn)

        # Display only 'name' and 'type' columns from the schema
        simplified_schema_df = schema_df[['name', 'type']]
        dic[selected_table] = simplified_schema_df
...
def initialSideBar(st,conn):
    with st.sidebar:
        st.header("Database Schema Explorer")
        
        # Display available tables (schemas)
        query = "SELECT name FROM sqlite_master WHERE type='table';"
        tables = pd.read_sql_query(query, conn)
        ...
        selected_table = st.selectbox("Select a table to view its schema:", tables['name'])

        if selected_table:
            # Query to get the schema of the selected table
            schema_query = f"PRAGMA table_info({selected_table});"
            schema_df = pd.read_sql_query(schema_query, conn)

            # Display only 'name' and 'type' columns from the schema
            simplified_schema_df = schema_df[['name', 'type']]
            print(simplified_schema_df)
            st.write(f"Schema for table `{selected_table}`:")
            st.table(simplified_schema_df)
...
def simpleSideBar(st,conn):
    with st.sidebar:
        ...
        for key, table in dic.items():
            ...
            simplified_schema_df = table[['name', 'type']]
            st.table(simplified_schema_df)
